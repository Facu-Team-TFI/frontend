...
const Profile = () => {
    ...
    const fetchProfile = async () => {
      if (id) {
        ...
        setSellerData({
          ...
          //   profileImage: user.avatarUrl?.startsWith("/")
          //     ? user.avatarUrl
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      setUser(updatedUser);
      setSellerData((prev) => ({ ...prev, profileImage: resData.avatarUrl })); //Agergado
    } catch (error) {
    ...
    <>
      <div className="min-w-full mx-auto bg-[#FDE7B9] p-9 rounded-2xl mt-12 relative">
        ...
        <div className="flex flex-col items-center">
          ...
          <h2 className="text-3xl mt-5 text-[#401809]">
            {sellerData.name} {sellerData.lastname}
          </h2>
          <span className="text-[#363738] text-[18px] ">
            @{sellerData.nickname}
          </span>
